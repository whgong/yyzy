--drop PROCEDURE YYZY.P_YYZY_LSPF_SJDHB6; 
...
CREATE PROCEDURE YYZY.P_YYZY_LSPF_SJDHB6 
( 
  IN IP_I_PFPHDM INTEGER
)
  SPECIFIC PROC_YYZY_LSPF_SJDHB6
  LANGUAGE SQL
  NOT DETERMINISTIC
  NO EXTERNAL ACTION
  MODIFIES SQL DATA
...
--  END; 
  ...
  for v1 as c1 cursor for 
    select PFPHDM, JSDM, YYDM, YYNF, SYRQ, YYSYL, ZXSX, 
      ZLYYBJ, ZPFBJ, KCLX 
  ...
  do --body of 轮询全部历史配方日数据
    if lc_i_rn = 1 then --是第一条
      ...
      if lc_ar_PFPHDM[lc_i_rn-1] = v1.pfphdm and lc_ar_JSDM[lc_i_rn-1] = v1.jsdm 
          ...
          and lc_ar_KCLX[lc_i_rn-1] = v1.kclx 
          --与上一条配方角色烟叶相同
      then 
        goto lb_update; 
      else --与上一条配方角色烟叶不同
    ...
    lb_new: --新增记录 
      ...
      set lc_ar_KCLX[lc_i_rn] = v1.kclx; 
  ...
  --数据入库
  delete from YYZY.T_YYZY_ZXPF_LSB where pfphdm = IP_I_PFPHDM; 
  insert into YYZY.T_YYZY_ZXPF_LSB( 
      PFPHDM, JSDM, YYDM, YYNF, KSRQ, JSRQ, YYFPL, ZXSX, 
      KSSYL, JSSYL, ZLYYBJ, ZPFBJ, KCLX 
    ) 
  ...
  from 
    unnest ( 
      ...
      lc_ar_KSSYL, lc_ar_JSSYL, lc_ar_ZLYYBJ, lc_ar_ZPFBJ, lc_ar_KCLX 
    ) as t(pfphdm, jsdm, yydm, yynf, ksrq, jsrq, 
          yyfpl, zxsx, kssyl, jssyl, zlyybj, zpfbj, kclx) 
  ; 
  
END LB_MAIN; 

COMMENT ON PROCEDURE YYZY.P_YYZY_LSPF_SJDHB6 (INTEGER) IS '历史配方时间段合并 6要素';
