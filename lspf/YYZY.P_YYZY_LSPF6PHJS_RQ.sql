--drop PROCEDURE YYZY.P_YYZY_LSPF6PHJS_RQ; 

SET SCHEMA = ETLUSR; 
SET CURRENT PATH = SYSIBM,SYSFUN,SYSPROC,ETLUSR; 

CREATE PROCEDURE YYZY.P_YYZY_LSPF6PHJS_RQ 
( 
  ...
  SPECIFIC PROC_YYZY_LSPF6PHJS_RQ 
  LANGUAGE SQL 
  NOT DETERMINISTIC 
  NO EXTERNAL ACTION 
  MODIFIES SQL DATA 
...
--  ) with replace on commit preserve rows not logged; 
  ...
  -------------------------------------------------------------------------------
  --判断核销日期是否在最后
  if (select count(*) from YYZY.T_YYZY_ZXPF_LSB_R 
  ...
  if lc_n_yyhyl = 0 then --若当天的耗用为0，沿用历史
    ...
    ; 
    select YYDM, YYNF, ZLYYBJ, ZPFBJ, KCLX 
        into lc_c_yydm_k, lc_i_yynf_k, lc_c_ZLYYBJ, lc_c_ZPFBJ, lc_i_kclx_k 
  ...
  for v1 as c1 cursor for 
    select pfphdm, jsdm, yydm, yynf, kclx, yyfpl, zlyybj, zpfbj, tdsx 
  ...
  do --body of 轮询锁定部分数据
    ...
    if v1.yyfpl > lc_n_yyhyl then --若获取的烟叶的锁定数量足够
      --缓存烟叶信息
      values(v1.yydm, v1.yynf, v1.ZLYYBJ, v1.zpfbj, v1.kclx, lc_n_yyhyl) 
        into lc_c_yydm_k, lc_i_yynf_k, lc_c_ZLYYBJ, lc_c_ZPFBJ, lc_i_kclx_k, lc_n_yyfpl 
      ; 
      --烟叶的锁定数量 - lc_n_yyhyl
      ...
      --缓存烟叶信息
      values(v1.yydm, v1.yynf, v1.ZLYYBJ, v1.zpfbj, v1.kclx, v1.yyfpl) 
        into lc_c_yydm_k, lc_i_yynf_k, lc_c_ZLYYBJ, lc_c_ZPFBJ, lc_i_kclx_k, lc_n_yyfpl
      ;
      --待分配量减少v1.yyfpl
  ...
  lb_data_input:begin --历史配方数据入库
    ...
    set lc_n_zxsx = coalesce( 
        (select sum(YYSYL) from YYZY.T_YYZY_ZXPF_LSB_R 
          where pfphdm = IP_PFPHDM and jsdm = IP_JSDM and syrq = IP_RQ 
        ),0 
      ) 
    ; 
    merge into YYZY.T_YYZY_ZXPF_LSB_R as t 
    using ( 
        select m.YYDM, m.YYNF, m.KCLX, m.zlyybj, m.zpfbj, 
          m.yyfpl, coalesce(l.zxsx,-1) as zxsx 
        from 
          (values (lc_c_yydm_k, lc_i_yynf_k, lc_i_kclx_k, 
              lc_c_ZLYYBJ, lc_c_ZPFBJ, lc_n_yyfpl)) 
            as m(YYDM, YYNF, KCLX, zlyybj, zpfbj, yyfpl) 
          left join (
            select YYDM, YYNF, KCLX, zxsx 
            from YYZY.T_YYZY_ZXPF_LSB_R 
            where pfphdm = IP_PFPHDM and jsdm = IP_JSDM and syrq = IP_RQ 
            order by SYRQ desc,ZXSX desc
            fetch first 1 row only 
    ...
    when not matched then 
      insert (PFPHDM, JSDM, YYDM, YYNF, SYRQ, YYSYL, 
              ZXSX, ZLYYBJ, ZPFBJ, KCLX
              )
      values (IP_PFPHDM, IP_JSDM, s.yydm, s.yynf, IP_RQ, s.yyfpl, 
                lc_n_zxsx, s.zlyybj, s.ZPFBJ, s.KCLX 
              )
    ; --end of 数据入库脚本 
    ---------------------------------------------------------------
    --判断状态，处理后续
...
COMMENT ON PROCEDURE YYZY.P_YYZY_LSPF6PHJS_RQ( 
    integer, integer, date, decimal(18,6) 
  ) IS '6要素历史配方 牌号角色 指定日期'
; 
